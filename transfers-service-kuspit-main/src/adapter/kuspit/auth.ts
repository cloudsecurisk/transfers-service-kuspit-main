...
export async function linkClient(params: KuspitLinkParams): Promise<KuspitLinkResponse> {
  ...
  if (
    !Number.isInteger(params.state)
    || params.state.toString().length !== 10
  ) {
    throw new AppError(400, 'State must be a 10-digit number')
  }

  // Query params (not sensitive)
  ...
  // Body as form-urlencoded (sensitive)
  ...
  return makeRequest<KuspitLinkResponse>({
    method: 'POST',
    endpoint: `v3/authorization?${queryParams}`,
    params: bodyParams,
  })
}
