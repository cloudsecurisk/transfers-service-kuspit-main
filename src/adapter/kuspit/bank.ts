...
export interface LegacyJavaDate {
  time: number
  minutes?: number
  seconds?: number
  hours?: number
...
export interface MovementsResponse {
  movimientos: Array<{
    ...
    claveRastreo: string
    concepto: string
    fechaLiquidacion: LegacyJavaDate // yyyy-MM-dd
    monto: number
    comision: number
    iva: number
    estatus: string
    fechaOrden: LegacyJavaDate
    bancoEmisor: number
...
export interface LiquidAssetsParams {
  contrato: number
  empresa: string
}
export interface LiquidAssetsResponse {
  saldoLiquido: number
}
...
export async function getAccountMovements(
  ...
  return makeRequest<MovementsResponse>({
    method: 'GET',
    endpoint: 'v3/banca/consultas/movimientos',
    params: queryParams,
    token: params.token,
  })
}
...
export async function updateBankInfo(
  ...
  const fileWithProps = params.caratulaCuentaClaBe as unknown as { name?: string, type?: string }

  let finalFileName = fileWithProps.name

  if (!finalFileName) {
    const mimeType = fileWithProps.type || ''
    let ext = 'pdf'

    if (mimeType.includes('png')) ext = 'png'
    else if (mimeType.includes('jpeg') || mimeType.includes('jpg')) ext = 'jpg'

    finalFileName = `caratula_bancaria.${ext}`
  }

  logger.info(`Uploading file with name: ${finalFileName}`)
  logger.info(`File details - Size: ${params.caratulaCuentaClaBe.size} bytes, Type: ${params.caratulaCuentaClaBe.type}`)

  formData.append('caratulaCuentaClaBe', params.caratulaCuentaClaBe, finalFileName)
...
/**
 * Get the liquid assets of the account.
 * @param {LiquidAssetsParams} params - Parameters for the liquid assets query.
 * @returns {Promise<LiquidAssetsResponse>} The liquid assets of the account.
 */
export async function getLiquidAssets(
  params: LiquidAssetsParams & { token: string },
): Promise<LiquidAssetsResponse> {
  const queryParams = new URLSearchParams({
    contrato: params.contrato.toString(),
    empresa: params.empresa,
  })

  return makeRequest<LiquidAssetsResponse>({
    method: 'GET',
    endpoint: '/v1/banca/consultas/saldo-liquido',
    params: queryParams,
    token: params.token,
  })
}
